...
window.addEventListener('DOMContentLoaded', () => {
    ...
    /**
     * 2. 모든 입력창(인증 및 비밀번호 변경) 엔터 이벤트 통합
     */
    // 로그인/가입 박스들과 비밀번호 변경 모달 내의 모든 input을 선택
    const allAuthInputs = document.querySelectorAll('.login-box input');
    
    allAuthInputs.forEach(input => {
        input.addEventListener('keydown', function(e) {
            if (e.key === 'Enter') {
                // 1. 일반 로그인/회원가입/찾기 박스 확인
                const parentBox = this.closest('.login-box');
                if (!parentBox) return;

                const boxId = parentBox.id;
                
                // 2. 만약 부모 박스에 ID가 없다면 (비밀번호 변경 모달인 경우)
                // 비밀번호 변경 모달은 최외곽 div에 ID가 있으므로 이를 체크합니다.
                const isChangePwModal = this.closest('#change-pw-zone');
                ...
                if (isChangePwModal) {
                    ...
                    attemptRegister();
                } else if (boxId === 'find-id-box') {
                    attemptFindId();
                } else if (boxId === 'find-pw-box') {
                    attemptFindPw();
                }
            }
        });
...
function switchAuthView(viewName) { 
    const boxes = ['login-box', 'register-box', 'find-id-box', 'find-pw-box'];
    boxes.forEach(id => document.getElementById(id).classList.add('hidden'));

    const targetId = `${viewName}-box`;
    const targetBox = document.getElementById(targetId);
    if (targetBox) {
        targetBox.classList.remove('hidden');
        const firstInput = targetBox.querySelector('input');
...
async function requestAiRecommendation(isInitial = false) {
    ...
    try {
        ...
        if (data.status === 'success' || data.status === 'no_more') {
            if (data.current_min_price !== undefined) currentMinPrice = data.current_min_price;
            if (data.current_max_price !== undefined) currentMaxPrice = data.current_max_price;
            ...
            if (data.status === 'success') {
                ...
                appendChat('bot', `💡 오늘의 추천은 <b>[${store.store_name}]</b>의 <b>[${data.food}]</b>입니다! (${Number(data.price).toLocaleString()}원)`);
            } else {
...
function resetRecommendationState() {
    ...
    // 2. 채팅 로그 초기화 및 안내 메시지
    const chatLog = document.getElementById('ai-chat-log');
    chatLog.innerHTML = `
        <div class="chat-msg chat-bot" style="margin-bottom: 8px;">
            <strong style="color: #ff6b6b;">AI 추천봇:</strong> 추천 조건과 이력이 깨끗하게 초기화되었습니다! 처음부터 다시 추천을 시작할게요. 🌟
        </div>
    `;

    // 3. 입력창 비우고 첫 추천 다시 트리거
